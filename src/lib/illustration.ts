...
/** One plate the planner recommends: the section it belongs under, and the one
 *  physical subject from that section to depict. */
...
/**
 * Decide which of a chapter's sections genuinely warrant a photographic plate —
 * the fitness gate for the "maximum" visual-density tier.
 *
 * This is the difference between "densely illustrated" and "padded": rather than
 * force a fixed number of photos onto every chapter, it reads the finished
 * chapter and returns a plate only where a real, physical subject earns one,
 * skipping sections a photograph would only decorate. The count therefore
 * follows the content — many for a hands-on, object-rich chapter, few or none
 * for an abstract one.
 *
 * Returns [] on any failure, and a synthetic per-section plan in mock mode so a
 * mock run still previews the dense layout without spending anything. Never
 * throws: a chapter with no plan simply keeps its opener.
 */
export async function planChapterPlates(
	book: IllustrationBookContext,
	brief: IllustrationBrief,
	/** The opener's subject, so the plan avoids proposing the same picture twice. */
	alreadyDepicted: string,
	keys: IllustrationKeys,
	useMock: boolean
...
export async function createIllustration(
	...
	prompt: string;
	/** The one thing from the chapter this plate depicts, as chosen by the
	 *  art-director (empty when it fell back to the house style). Callers that
	 *  commission several plates for one chapter pass the earlier subjects back
	 *  in as an authorNote so each new plate picks something different. */
	subject: string;
