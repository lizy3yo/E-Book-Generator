...
export async function artDirectIllustration(
	...
	useMock: boolean
): Promise<{ prompt: string; subject: string } | null> {
	try {
		...
		return data.success && data.prompt
			? { prompt: data.prompt as string, subject: (data.subject as string) ?? '' }
			: null;
	} catch {
		return null;
	}
...
export async function labelIllustration(
	...
	try {
		...
		const data = await res.json();
		return data.success && Array.isArray(data.labels) ? (data.labels as IllustrationLabel[]) : [];
	} catch {
		return [];
	}
}
...
): Promise<{ url: string; labels: IllustrationLabel[]; prompt: string }> {
